...
public class TelaPrincipal extends JFrame {
    ...
    private void criarPainelAreas() {
        ...
        // Para adicionar uma nova área, basta incluir uma nova linha aqui!
        List<AreaCaca> areas = List.of(
                ...
                new AreaCaca("Mar Elétrico", 35, new Color(255, 255, 0),   Color.DARK_GRAY, () -> new Kjoule(),     "src/image/Gemini_Generated_Image_w0uvyow0uvyow0uv.png")
        );

        for (AreaCaca area : areas) {
            if (jogador.getNivel() >= area.nivelMinimo) {
                JButton btn = criarBotaoMenu(area.nome, area.corFundo, area.corTexto);
                btn.addActionListener(e -> iniciarCombate(area.criarInimigo(), area.cenario));
                painelAreas.add(btn);
            }
        }
    }
